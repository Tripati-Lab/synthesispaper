...
fitsinglePartitioned <- function(calData, targetColumns, replicates=2 , generations=1000, maxtry=10, export=T, prefix='results'){
  sumPart<-lapply(targetColumns, function(x){
    ...
    subSampled<-lapply(unique(calDataSelected$Material), function(y){
      ...
      k=0
      while ( keep ) {
        keep= if(i==0){if(k>maxtry ){F}else{T} }else{F}
        k=k+1
        tryCatch({
  ...
  invisible(lapply(seq_along(condensed), function(x) write.csv(condensed[[x]], paste0("Results/",prefix,'_',names(condensed)[x], '.csv')) ) )
  }else{
  return(condensed)
  }
}
